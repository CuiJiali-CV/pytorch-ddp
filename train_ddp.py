...
def main(local_rank, nprocs, args):
    ...
    netI = _netI(args['zi_dim']).cuda(local_rank)
    netI = DistributedDataParallel(netI, device_ids=[local_rank])
    average_params(netI.parameters(), is_distributed=True)  # syn params

    logging.info(f"netG has parameters {count_parameters_in_M(netG)}")
    logging.info(f"netI has parameters {count_parameters_in_M(netI)}")

    optG = torch.optim.Adam(netG.parameters(), lr=args['g_lr'])
    optI = torch.optim.Adam(netI.parameters(), lr=args['i_lr'])

    train(netG, netI, optG, optI, train_dl, local_rank, nprocs, logging, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vis_iter", type=int, default=10)
    parser.add_argument("--compute_fid", type=int, default=0)
    parser.add_argument("--fid_start_ep", type=int, default=5)
    parser.add_argument("--iter_metric", type=int, default=1)

    parser.add_argument("--temperature", type=float, default=1.)

    parser.add_argument("--EBM_type", type=str, default="simple", help="simple, ResNet_small, ResNet_medium, ResNet_NCP")

    parser.add_argument("--nef", type=int, default=64)
    parser.add_argument("--ndf", type=int, default=200)
    parser.add_argument("--e_lr", type=float, default=3e-4)
    parser.add_argument("--wd", type=float, default=3e-5)
    parser.add_argument("--sample_steps", type=int, default=10)
    parser.add_argument("--step_size", type=float, default=4e-3)
    ...
    args.save = get_output_dir(__file__, added_directory=str(args.checkpoint).upper() + '/' + str(args.EBM_type))
    os.makedirs(args.save, exist_ok=True)

    args = check_args(args)

    copy_source(__file__,  args.save)
